...
public sealed class ActivateUserCommandHandler : IRequestHandler<ActivateUserCommand, Result>
{
    ...
    public async Task<Result> Handle(ActivateUserCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogDebug("Activate user rejected: caller is not an administrator.");
        ...
        {
            _logger.LogDebug("Activate user rejected: caller attempted to activate their own account.");
            return Result.Failure(
                UserLifecycleErrorCodes.CannotActOnSelf,
                "You cannot change activation state on your own account with this action.");
        }

        User? user = await _userLogin.GetTrackedByIdAsync(request.UserId, cancellationToken).ConfigureAwait(false);
        if (user is null)
        {
            _logger.LogWarning("Activate user: no user record for id {UserId}.", request.UserId);
        ...
        {
            _logger.LogDebug(
                "Activate user rejected: caller role {CallerRole} cannot manage target role {TargetRole}.",
                _currentUser.Role,
                user.Role);

            return Result.Failure(
                UserLifecycleErrorCodes.InsufficientPrivilegeForTarget,
                "You cannot reactivate this account type.");
        }

        user.Activate();

        _logger.LogInformation(
            "User {UserId} ({Email}) activated by {ActorId} ({ActorRole}).",
            user.Id,
            user.Email,
            _currentUser.UserId,
            _currentUser.Role);
