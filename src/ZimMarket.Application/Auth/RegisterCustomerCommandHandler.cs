...
public sealed class RegisterCustomerCommandHandler : IRequestHandler<RegisterCustomerCommand, Result<AuthTokensDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogDebug("Customer registration rejected: invalid phone format.");
            return Result<AuthTokensDto>.ValidationFailure(
            [
                new ValidationError(
                    nameof(RegisterCustomerCommand.Phone),
        ...
        string? pushToken = string.IsNullOrWhiteSpace(request.PushToken) ? null : request.PushToken.Trim();

        var hashSource = new Customer(
            id,
            normalizedEmail,
            fullName,
            phone,
        ...
        customer.SetRefreshToken(refreshHash, _jwtService.GetRefreshTokenExpiresAtUtc());
        ...
        await _unitOfWork.Customers.AddAsync(customer, cancellationToken).ConfigureAwait(false);
