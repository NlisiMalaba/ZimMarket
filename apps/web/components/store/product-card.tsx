...
export function ProductCard({ product, priority }: { product: StorefrontProduct; priority?: boolean }) {
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-[var(--radius-card)] border border-border bg-page-elevated shadow-[var(--shadow-card)] transition duration-300 hover:-translate-y-0.5 hover:border-border-strong hover:shadow-[var(--shadow-card-hover)]">
      <Link href={href} className="relative block aspect-square overflow-hidden bg-page">
        ...
        {product.badge ? (
          <span className="absolute left-3 top-3 rounded-full bg-cta px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-white shadow-sm">
            {product.badge === "deal" ? "Deal" : product.badge === "new" ? "New" : "Trending"}
          </span>
        ) : null}
        {pct ? (
          <span className="absolute right-3 top-3 rounded-full bg-success px-2.5 py-1 text-[11px] font-semibold text-white shadow-sm">
            -{pct}%
          </span>
        ) : null}
      </Link>
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          {product.verifiedSeller ? (
            ...
            <span className="inline-flex items-center gap-1 rounded-full border border-border px-2 py-0.5 text-[11px] font-medium text-muted">
              Marketplace seller
            </span>
          )}
          {product.warehouseVerified ? (
            <span className="inline-flex items-center gap-1 rounded-full border border-brand/20 bg-brand/5 px-2 py-0.5 text-[11px] font-medium text-brand">
              Warehouse checked
            </span>
          ) : null}
        </div>
        ...
        <div className="mt-4 flex gap-2">
          <Link
            href={href}
            className="inline-flex flex-1 items-center justify-center rounded-[12px] border border-border bg-page px-3 py-2 text-xs font-semibold text-foreground transition hover:border-brand hover:text-brand"
          >
            View
          </Link>
          <Link
            href="/cart"
            className="inline-flex flex-1 items-center justify-center rounded-[12px] bg-cta px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-cta-hover"
          >
            Buy now
          </Link>
        </div>
