...
const initialFormState: RegisterSellerFormState = {
  fullName: '',
  businessName: '',
  email: '',
  phone: '',
...
export default function RegisterSellerScreen() {
  ...
  const [form, setForm] = useState<RegisterSellerFormState>(initialFormState);
  ...
  const handleChange = (key: keyof RegisterSellerFormState, value: string) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.replace('/(seller)/kyc-upload' as never);
    } catch {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Text style={styles.primaryButtonText}>{isLoading ? 'Creating account...' : 'Continue to KYC upload'}</Text>
      </Pressable>
    </View>
  );
}
