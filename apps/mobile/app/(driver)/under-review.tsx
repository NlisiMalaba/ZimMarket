...
export default function DriverUnderReviewScreen() {
  ...
  useEffect(() => {
    ...
    const pollStatus = async () => {
      try {
        const response = await driverOnboardingService.pollKycStatus(accessToken, refreshToken);
        if (!isMounted) {
          return;
        }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.description}>
        Your driver documents were submitted successfully. We are checking your status every 10 seconds.
      </Text>
      ...
      {status.trim().toLowerCase() === 'approved' ? (
        <Pressable style={styles.primaryButton} onPress={() => router.replace('/(driver)')}>
          <Text style={styles.primaryButtonText}>Continue to driver home</Text>
        </Pressable>
      ) : null}
